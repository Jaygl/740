...
tic1 = time.time()
print('Made it through setup')

""" Begin main algorithm """
for idx in range(0, gl.numGoals):                      # for each goal
    L = fcn.setupLevels()
    ...
    while gl.start != gl.goal:
        ...
        print(L)
        import pdb; pdb.set_trace()  # breakpoint 5e56c870 //
        ...
        while not goalMoved and validPath and gl.start != gl.goal and path:
            ...
            for point in path:
                ...
                if useMovingGoals:
                    for i in range(0,len(initX)):  goalMoved = fcn.movingGoal(initX[i], initY[i], initZ[i], T[i])
