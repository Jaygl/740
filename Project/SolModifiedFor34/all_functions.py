...
def rectObs(locX, locY, locZ, dimX, dimY, dimZ):
    ...
    for dx in range(0, dimX):
        for dy in range(0, dimY):
            for dz in range(0, dimZ):
...
def plotRectObs(x, y, z, xd, yd, zd, alpha, axisname):
    ...
    poly3d = [[tupleList[vertlist[ix][iy]] for iy in range(len(vertlist[0]))] for ix in range(len(vertlist))]
...
def setupLevels():
    ...
    for level in range(gl.numlevels,-1,-1):
...
def searchAndUpdate(xNew,yNew,zNew,*args):
    ...
    [sr_append((dx,dy,zmin)) for dx in range(xmin, xmax+1) for dy in range(ymin, ymax+1)]
    # Face 2: vary x,y at zmax
    [sr_append((dx,dy,zmax)) for dx in range(xmin, xmax+1) for dy in range(ymin, ymax+1)]
    # Face 3: vary x,z at ymin
    [sr_append((dx,ymin,dz)) for dx in range(xmin, xmax+1) for dz in range(zmin, zmax+1)]
    # Face 4: vary x,z at ymax
    [sr_append((dx,ymax,dz)) for dx in range(xmin, xmax+1) for dz in range(zmin, zmax+1)]
    # Face 5: vary y,z at xmin
    [sr_append((xmin,dy,dz)) for dy in range(ymin, ymax+1) for dz in range(zmin, zmax+1)]
    # Face 6: vary y,z at xmax
    [sr_append((xmax,dy,dz)) for dy in range(ymin, ymax+1) for dz in range(zmin, zmax+1)]
    ...
    if args:
        ...
        if len(path) > 0:
            ...
            for idx in range(len(path_section)-1):
...
def searchAndUpdate_old(xNew,yNew,zNew,*args):
    ...
    [sr_append((dx,dy,dz)) for dx in range(xmin, xmax+1) for dy in range(ymin, ymax+1) for dz in range(zmin, zmax+1)]
...
def simulateUAVmovement(pathToFollow):
    ...
    """
    nextcoords = []
    for k in range(len(pathToFollow)-1):
        ...
        if maxDist <=1:
            ...
            for jj in range(1, int(maxDist+1)):
...
def findPath(L):
    ...
    if level != 0 and time.clock()-startTime < pathComputationLimit:
        ...
        for newlevel in range(level-1,-1,-1):
